        ...
        $bdd = new connectionDb();
        $req = $bdd->db->prepare('SELECT idUtilisateur, motdepasse, statut FROM utilisateur WHERE pseudo = :pseudo');
        $req->execute(array(
          'pseudo' => $_POST['pseudo']));
        $resultat = $req->fetch();
        
        $isPasswordCorrect = password_verify($_POST['password'], $resultat['motdepasse']);
        ...
        else
        {
          if($isPasswordCorrect){
            ...
            echo '<div class="alert alert-dismissible alert-danger"> Mauvais identifiant ou mot de passe!</div>';
          }
        }}
        ?>
<div class="pos-f-t">
    ...
    <div class="bg-dark p-4">
      <ul class="nav nav-pills flex-column">
        ...
        if(isset($_SESSION['statut']) && $_SESSION['statut']==1){
          echo '<li class="nav-item flex-column">Bienvenue '.$_SESSION['pseudo'].'
          <span class="badge badge-pill badge-success">Membre</span></li>';
        }elseif(isset($_SESSION['statut']) && $_SESSION['statut']==2){
          echo '<li class="nav-item flex-column">Bienvenue '.$_SESSION['pseudo'].'
          <span class="badge badge-pill badge-warning">Administrateur</span></li>';
        };
        ?>
        <li class="nav-item">
        ...
        <li class="nav-item">
          <a class="nav-link text-white" href="contact.php">Infos</a>
        </li>
        <?php 
        if(isset($_SESSION['statut'])){
          echo '<li class="nav-item">
          <a class="nav-link text-white" href="gestion.php">Gestion</a>
          </li>';
        };
        ?>
        </ul>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalCenterTitle">Authentification</h5>
...
if(isset($_SESSION['idUtilisateur']) AND isset($_SESSION['pseudo'])){
  ...
  <form method="post" action=""><input type="submit" name="deconnexion" class="btn btn-primary btn-lg btn-block" value="Se déconnecter"></form>';
  ...
  echo '
        ...
        <div class="modal-footer">
          <input name="connexion" type="submit" class="btn btn-primary" value="Connexion">
        </form>
              <form>
                <button type="submit" class="btn btn-secondary" formaction="inscription.php">Inscription</button>
              </form>
      ';
